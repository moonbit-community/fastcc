...
// Patch note:
// - Original mbtcc ctest2/complex_function3.c had an invalid compose_map()
//   implementation (assigning through a function pointer).
// - Keep the test intent (higher-order functions + composition) while making it
//   valid C for gcc/clang and tinycc.mbt, aligning with refs/tinycc first.
...
// Note: The original test never calls compose_map(). Keep it as a simple,
// valid stub to avoid gcc/clang compilation errors without introducing extra
// object-format edge cases.
