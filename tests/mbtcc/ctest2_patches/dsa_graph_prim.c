...
// Patch note:
// - Original mbtcc ctest2/dsa_graph_prim.c has undefined behavior for
//   disconnected graphs: min_index and parent[] can stay uninitialized.
// - Make the output deterministic (no crash) so gcc/clang baseline is valid.
// - Keep algorithm/output structure as close as possible to the original.
...
int min_key(int key[], int mst_set[], int V) {
    ...
    if (min_index == -1) {
        // Disconnected remainder: pick the first vertex not in MST.
        for (int v = 0; v < V; v++) {
            if (mst_set[v] == 0) {
                min_index = v;
                break;
...
void prim_mst(struct Graph* graph) {
    ...
    // Initialize all keys as INFINITE and parent as "unset".
    for (int i = 0; i < V; i++) {
        key[i] = INF;
        mst_set[i] = 0;
        parent[i] = -1;
