...
// Patch note:
// - Original mbtcc ctest2/dsa_dp_edit_distance.c prints uninitialized dp[][],
//   making gcc/clang baseline output non-deterministic / mismatched vs refs/tinycc.
// - Initialize dp[][] to make prints deterministic, preserving the test intent.
...
int edit_distance(const char* str1, const char* str2) {
    ...
    // Initialize the whole printed region to make intermediate prints deterministic.
    for (int i = 0; i <= m; i++) {
        for (int j = 0; j <= n; j++) {
            dp[i][j] = 0;
        }
